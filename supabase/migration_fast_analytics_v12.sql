-- V12: Restore full-year data and optimize result set
-- Please run this in Supabase SQL Editor

DROP FUNCTION IF EXISTS get_product_sales_stats(DATE, INTEGER, INTEGER);
...
BEGIN
    ...
    WITH target_barcodes AS (
        SELECT p.barcode
        FROM products p
        -- OPTIMIZATION: Filter out completely dead products to avoid cluttering UI with 30k rows
        -- Only show if they have stock OR any sales this year
        -- But for "All" view, maybe user wants everything? Let's keep it inclusive but sorted.
        ORDER BY p.barcode
    ...
    daily_consolidated AS (
        ...
        WHERE ds.date >= start_of_year - integer '7' -- a bit before for week trends
