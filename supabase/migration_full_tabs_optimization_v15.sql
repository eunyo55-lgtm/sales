-- 1. Optimized Product Stats RPC (V15)
CREATE OR REPLACE FUNCTION get_product_stats_v15(
...
-- 2. Optimized Supply Analytics RPC (V15)
CREATE OR REPLACE FUNCTION get_supply_analytics_v15(
...
BEGIN
    ...
    upcoming_orders AS (
        SELECT 
            ...
            p.name as prod_name,
            p.image_url as prod_image,
            p.option_value as prod_option,
            p.cost as prod_cost
        FROM coupang_orders o
        LEFT JOIN products p ON o.barcode = p.barcode
        WHERE o.received_qty = 0 AND o.confirmed_qty >= 1 AND o.order_date >= CURRENT_DATE - interval '1 day'
    )
    ...
    SELECT json_agg(u) INTO upcoming_json FROM upcoming_orders u;

    RETURN json_build_object(
        'timeline', COALESCE(timeline_json, '[]'::json),
        'performance', COALESCE(performance_json, '[]'::json),
        'upcoming', COALESCE(upcoming_json, '[]'::json)
    );
END;
$$;
