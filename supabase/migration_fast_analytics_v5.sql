-- V5: Complete removal of raw data downloads via JSON object aggregation
-- Please run this in Supabase SQL Editor

-- Drop the existing function first because we are changing its return type
...
BEGIN
    ...
    SELECT 
        ...
        json_object_agg(d.date::TEXT, COALESCE(d.quantity, 0)) FILTER (WHERE d.date > anchor_date - integer '30' AND d.date <= anchor_date) AS daily_sales,
        json_object_agg(d.date::TEXT, COALESCE(d.stock, 0)) FILTER (WHERE d.date > anchor_date - integer '30' AND d.date <= anchor_date AND d.stock IS NOT NULL) AS daily_stock
