...
import { Search, ArrowUpDown, Loader2, ChevronRight, ChevronDown, Filter, CheckCircle2 } from 'lucide-react';
        ...
        <table className="min-w-full text-sm text-left border-collapse">
          <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-100 sticky top-0 z-30 shadow-sm">
            <tr>
              ...
              <th className={`px-4 py-3 bg-gray-50 sticky z-30 ${W_NAME} ${L_NAME} cursor-pointer hover:bg-gray-100 whitespace-nowrap shadow-[4px_0_4px_-4px_rgba(0,0,0,0.1)]`} onClick={() => handleSort('name')}>
                상품명 <ArrowUpDown size={12} className="inline ml-1 opacity-50" />
              </th>

              <th className="px-4 py-3 text-right bg-blue-50 cursor-pointer hover:bg-blue-100 whitespace-nowrap" onClick={() => handleSort('hqStock')}>본사재고 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
              <th className="px-4 py-3 text-right bg-green-50 cursor-pointer hover:bg-green-100 whitespace-nowrap" onClick={() => handleSort('coupangStock')}>쿠팡재고 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>

              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 whitespace-nowrap" onClick={() => handleSort('sales14Days')}>14일 판매 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 font-bold whitespace-nowrap" onClick={() => handleSort('sales7Days')}>7일 판매 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 whitespace-nowrap" onClick={() => handleSort('salesYesterday')}>전일 판매 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 whitespace-nowrap" onClick={() => handleSort('avgDailySales')}>일평균 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>

              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 text-blue-600 whitespace-nowrap" onClick={() => handleSort('estimatedSales7d')}>예상(7일) <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 text-red-600 whitespace-nowrap" onClick={() => handleSort('minDaysOfInventory')}>소진 예상 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
              <th className="px-4 py-3 text-right cursor-pointer hover:bg-gray-100 whitespace-nowrap" onClick={() => handleSort('shortage')}>부족수량 <ArrowUpDown size={12} className="inline ml-1 opacity-50" /></th>
